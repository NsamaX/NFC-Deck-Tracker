...
class RecordBloc extends Bloc<RecordEvent, RecordState> {
  ...
  void _onGetCardFromRecord(
      GetCardFromRecordEvent event, Emitter<RecordState> emit) {
    final record =
        state.records.firstWhere((r) => r.recordId == event.recordId);
    final cards =
        getCardFromRecordUsecase.call(record: record, deck: event.deck);
  ...
  PlayerAction getLastAction({
    ...
    final data = state.currentRecord.data
        .where((e) => e.collectionId == collectionId && e.cardId == cardId)
        .toList();
