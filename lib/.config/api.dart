...
class ApiConfig {
  ...
  static ApiConfig get instance {
    if (_instance == null) {
      ...
      throw StateError(
          'ApiConfig has not been initialized. Call ApiConfig.load() first.');
    }
    return _instance!;
  }
  ...
  static Future<void> load(String environment) async {
    ...
    try {
      ...
      if (envData == null) {
        throw ArgumentError(
            'Environment "$environment" not found in GameConfig');
      }
      ...
      _instance = null;
      LoggerUtil.e('Failed to load API config for "$environment"',
  ...
  String getBaseUrl(String key) {
    ...
    if (url == null || url.isEmpty) {
      LoggerUtil.w(
          'Base URL not found for key "$key" in environment "$environment"');
      throw ArgumentError(
          'Base URL not found or empty for "$key" in environment "$environment"');
    }
    return url;
  }
}
